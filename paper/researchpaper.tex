...
\begin{itemize}
    ...
    \item Evaluation metrics: Recall@10, true recall (brute-force for datasets up to 1M vectors), latency (ms), memory (GB), index size (MB)
\end{itemize}
...
This work demonstrates that agentic AI systems can effectively optimize low-level vector database
parameters by combining structured workflows with LLM-based reasoning. Our design addresses the
multi-objective trade-offs between recall, latency, and memory through a simple phase-aware strategy
and lightweight workload profiles. The system demonstrates that a single agent can discover
qualitatively different configurations for reasoning-oriented versus reaction-oriented databases.

The current prototype has several limitations. Large-scale experiments remain computationally
expensive, true recall is approximated heuristically for the largest datasets, and exploration is
driven by a single agent. Future work could explore richer exploration strategies such as
reinforcement learning or multi-agent setups, broader support for other FAISS index types, and
tighter integration with GPU acceleration and real-time workload monitoring.

\section{Conclusion}

We presented an agentic AI system that autonomously tunes FAISS HNSW vector database parameters using language-model-guided proposals and iterative measurement. By structuring optimization into recall and latency phases and encoding simple workload profiles, the system discovers distinct configurations for Knowledge + Reasoning and Memory + Reaction databases that reflect their different tolerance for latency and recall loss.

On synthetic datasets with $10^6$ vectors, the agent achieves recall@10 values of at least $0.90$ for Knowledge + Reasoning and around $0.70$ with sub-millisecond latency for Memory + Reaction, typically within 5 iterations. These results show that a lightweight agent, coupled with carefully designed prompts and logging, can approach expert-level tuning without exhaustive search.

Beyond the specific numbers, the framework cleanly separates measurement, reasoning, and decision logic, logs experiments for cross-run learning, and provides hooks for extending to other FAISS index types or real-world workloads such as RAG and recommendation. This provides a practical blueprint for self-optimizing vector database systems that adapt to evolving workload requirements.
